...
func (r *router) findMdls(root *node, segs []string) []Middleware {
	...
	for len(queue) != 0 {
		...
		queue = queue[1:]
		if len(tmp.mdls) > 0 {
			middlewares = append(middlewares, tmp.mdls...)
		}
		//if tmp.path=="*"{
		//	middlewares=append(middlewares,tmp.mdls...)
		//}
		...
		//	middlewares=append(middlewares,tmp.mdls...)
		//}
		//if tmp.path==segs[layer-1]{
		//	middlewares=append(middlewares,tmp.mdls...)
		//}
		if layer == len(segs) {
			continue
		}
		//在将middleware添加完之后如果层数超过了segs数量就停止，不再遍历
		if layer > len(segs)-1 {
			break
		}
		//越具体越后调度
		if tmp.paramChild != nil {
			queue = append(queue, tmp.paramChild)
		}
		if tmp.starChild != nil {
			queue = append(queue, tmp.starChild)
		}
		//把当前节点的所有孩子放入队列
		if tmp.children != nil {
			if value, ok := tmp.children[segs[layer]]; ok {
